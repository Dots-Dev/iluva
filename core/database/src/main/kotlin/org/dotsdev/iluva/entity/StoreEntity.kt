...
import org.dotsdev.liuva.StoreLocation
import org.dotsdev.liuva.StoreSetting
...
class StoreEntity(id: EntityID<UUID>) : UUIDEntity(id) {
    ...
    fun toModel() = Store(
        ...
        tagline = tagline,
        logo = logo,
        setting = StoreSettingEntity.find { StoreSettingTable.store eq id }.first().toModel(),
        location = StoreLocationEntity.find { StoreLocationTable.store eq id }.first().toModel()
    )
}
